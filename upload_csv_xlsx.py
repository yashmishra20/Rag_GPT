...
def _prepare_db(uploaded_files):
    ...
    for uploaded_file in uploaded_files:
        ...
        if file_extension.lower() == ".csv":
            ...
            st.sidebar.warning(f"Unsupported file type: {file_extension}. Skipping {uploaded_file.name}")
            continue
...
def get_response(question, db_path):
...
def main():
    ...
    uploaded_files = st.sidebar.file_uploader("Upload CSV or XLSX files", accept_multiple_files=True, type=['csv', 'xlsx'])
    ...
    if uploaded_files:
        ...
        if process_button:
            with st.spinner("Processing uploaded files..."):
                st.session_state.db_path = _prepare_db(uploaded_files)
    ...
    if st.session_state.db_path:
        ...
        user_question = st.chat_input("Ask a question about your uploaded data:")
        if user_question:
            ...
            response = get_response(user_question, st.session_state.db_path)
        ...
        st.info("Please upload CSV or XLSX files using the sidebar and click 'Process Uploaded Files' to begin analysis.")
